...
void adicionarPizza(struct formatoPizzas *pizza, int* quantidadePizzas, struct ingredientes *ingrediente, int* quantidadeIG) {
	...
	int jatemessapizza=0;
	for (int i=0;i<*quantidadePizzas;i++){
	    if (strcmp(pizza[*quantidadePizzas].nome,pizza[i].nome)==0){
	        jatemessapizza=1;
	    }
	}
	if (jatemessapizza==0){
    	...
    	for (int i=0; i<pizza[*quantidadePizzas].quantidadeIngredientes; i++) {
    		...
    		pizza[*quantidadePizzas].ingredientes[i].id=(ingrediente[*quantidadeIG-1].id)+1;
    		
    		int temIngrediente=0;
    		for (int j=0;j<*quantidadeIG;j++){
    		    if (strcmp(ingrediente[j].nome,pizza[*quantidadePizzas].ingredientes[i].nome)==0){
    		        pizza[*quantidadePizzas].ingredientes[i].id=ingrediente[j].id;
    		        pizza[*quantidadePizzas].ingredientes[i].preco=ingrediente[j].preco;
    		        temIngrediente=1;
    		    }
    		}
    		if (temIngrediente==0){// é um ingrediente novo
    		    strcpy(ingrediente[*quantidadeIG].nome,pizza[*quantidadePizzas].ingredientes[i].nome);
    		    ingrediente[*quantidadeIG].id=pizza[*quantidadePizzas].ingredientes[i].id;
    		    ingrediente[*quantidadeIG].preco=pizza[*quantidadePizzas].ingredientes[i].preco;
    		    *(quantidadeIG)=*(quantidadeIG)+1;
    		}
    	}
	}else{
	    printf ("Ja existe uma pizza com esse nome\n");
	}
